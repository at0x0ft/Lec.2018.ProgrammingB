...
void rotate(int n, array src, array dst)
{
  ...
  for (j = 0; j < n; ) {
    ...
    for (i = 0; i < n; ) {
      ...
      for (j1 = j; j1 < j1_Lim; j1+=4) {
        int j2 = j1+1, j3 = j1+2, j4 = j1+3, dr1 = n-1-j1, dr2 = dr1-1, dr3 = dr1-2, dr4 = dr1-3;
        for (i1 = i; i1 < i1_Lim; i1+=4) {
          dst[dr1][i1] = src[i1][j1];
          dst[dr2][i1+1] = src[i1+1][j2];
          dst[dr3][i1+2] = src[i1+2][j3];
          dst[dr4][i1+3] = src[i1+3][j4];
        }
      }
      i = i1_Lim;
    }
